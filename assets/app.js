...
$(function(){
     $( ".product_gallery .gallery-cell" ).clone(true).appendTo( ".slide-placeholder" );
   //  $( ".product_gallery .gallery-cell" ).clone(true).appendTo( ".slide-placeholder-lightbox" );
...
var quickFilter = {
  init: function(){
    ...
    $.each(selectedOptions, function(i, value){
      if (i != selectedOptions.length - 1) {
        sizeTags += selectedOptions[i] + ',';
      } else {
        //Checks if it's price or size filter
        if(value.includes("0-")){
          priceMin = selectedOptions[i].split('-')[0];
          priceMax = selectedOptions[i].split('-')[1];
        }else{
          sizeTags += sizesMap[selectedOptions[i]];
        }
      }
    });
    ...
    quickFilter.processUrl(path, sizeTags, priceMin, priceMax, query);
  },
  ...
  processUrl: function(path, sizeTags=undefined, priceMin=undefined, priceMax=undefined, query){

    var query = query.replace(/\page=(\w+)&/, '');
    //     urlString = '';

    urlString = path + "?filter.v.availability=1";

    if (sizeTags) urlString += "&filter.v.option.size=" + sizeTags;
    if (priceMin && priceMax) urlString += "&filter.v.price.gte=" + priceMin + "&filter.v.price.lte=" + priceMax;
    if (sizeTags || (priceMin && priceMax)) query = query.replace('?', '&');
    urlString += query;
...
$(function() {
  ...
  utils.createAccordion('.footer_menu', 'h3', 'ul');
  utils.createAccordion('.footer_content', 'h3', 'div.toggle_content');
  ...
  function refreshCart(cart) {
    ...
    if (cart.item_count == 0) {
      ...
      $.each(cart.items, function(index, item) {
        // Ajax call to retrieve product's image instead of variant's image (color swatch)
        var featuredImage;
        $.ajax({
          dataType: "json",
          async: false,
        ...
        if (item.image) {
          cart_items_html += '<div class="cart_image">' +
              '<img src="' + item.image.replace(/(\.[^.]*)$/, "_compact$1").replace('http:', '') + '" alt="' + htmlEncode(item.title) + '" style="max-width: 100%"/>' +
            '</div>';
        } 
        /*if (item.image) {
          cart_items_html += '<div class="cart_image">' +
              '<img src="' + featuredImage.replace(/(\.[^.]*)$/, "_compact$1").replace('http:', '') + '" alt="' + htmlEncode(item.title) + '" style="max-width: 100%"/>' +
            '</div>';
        }*/
        ...
        if(item.properties) {
          $.each(item.properties, function(title, value) {
            if (value) {
              cart_items_html += '<div class="line-item">' + title + '</div>';
            }
          });
        }
